...
- **图片上下文兼容** — 跳过标记为不进入上下文的生图消息，并使用安全替代文本代替内部生图 Prompt
...
- **暗色主题** — 内置多色主题，根据 accent 颜色自动切换明暗，弹窗和卡片整体变色
- **服务端代理** — 通过 SillyTavern 后端代理 API 请求，彻底规避浏览器 CORS 限制（模型列表加载 + 摘要生成均走代理）
...
当其他扩展在消息的 `extra` 中写入以下字段时，本脚本会同步遵守相同的总结策略：

- `exclude_from_context: true`：该消息不计入自动触发数量、总结分段或手动总结内容。
- `context_message`：总结时使用该安全文本，例如 `[Sent a selfie.]`，不读取内部生图 Prompt。

本脚本只负责总结阶段的过滤，不会自行修改 SillyTavern 的聊天上下文或图片生成流程。图片是否进入聊天模型上下文，仍由 SillyTavern 和对应生图扩展决定。
...
- 支持 `exclude_from_context` 和 `context_message` 图片上下文策略。
- 排除消息不再占用自动总结触发数量或分段配额。
- 修复总结脚本恢复可见消息时可能清除其他扩展隐藏状态的问题。
- 手动总结与自动总结统一使用相同的安全消息过滤规则。
